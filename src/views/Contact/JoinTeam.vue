        ...
        <form @input="check($event)" class="borderless">
          ...
          <div class="row">
            <div class="field full">
              <input id="emailRecruit" class="input empty" type="email" placeholder="playboy007@caramail.com" required="true" autocomplete="email">
          ...
          <div class="row">
            ...
            <div class="field fill">
              <input id="phoneRecruit" class="input empty" type="text" placeholder="" required="true" autocomplete="mobile">
              <label for="phone">Mobile</label>
              <div class="baseline"></div>
            </div>
            <div class="field fill fourth">
              <input id="postalCodeRecruit" class="input empty fourth" type="text" placeholder="4000" required="true" autocomplete="postalCode">
          ...
          <div class="row resume">
            <div class="field fill fourth">
              <label class="resume" for="resume" data-tid="form.resume">Un CV ? Joins-le ici !</label>
            </div>
            <div class="field fill">
              <input ref="resume" id="resume" class="input empty" type="file" accept=".pdf, .docx, .jpg" @change="form.resume = $event.target.files[0]" hidden>
              <div class="preview">
                <p v-if="!!form.resume">Nom du fichier: <strong>{{form.resume.name}}</strong><span>{{`  `}}</span><span>{{` | `}}</span><span>{{`  `}}</span> Taille:  <strong>{{form.resume.size | toBytes}}</strong></p>
                <p v-else>Pas de fichier selectione</p>
              </div>
              <div class="baseline"></div>
            </div>
          </div>
